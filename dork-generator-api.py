...
python3 dork-generator-api.py --platform fofa --key YOUR_KEY --version 11.126.0 --json -o results.json
    ...
    # Search all versions
        ...
        for version in versions:
            ...
            if args.platform == "fofa":
                gen.fofa_search(dork, max_pages=50)  # Limit pages untuk bulk search
            elif args.platform == "shodan":
                gen.shodan_search(dork)
    ...
    # Batch search
        ...
        try:
           ...
           st print(f"{C.RED}[!] Error: {e}{C.RESET}")
    
    else:
        parser.print_help()
        print(f"\n{C.YELLOW}Tip: Use --list untuk lidrzdst semua vulnerable versions{C.RESET}\n") ss
dsxsyy TT TT ff fff gg di DC
